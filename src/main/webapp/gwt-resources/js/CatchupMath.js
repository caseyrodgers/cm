...
/** Called from GWT to mark correct answers
 * 
 * @return
 */
 ...
 *  one call func to select the correct answer
 *  
 * @return
 */
window.showCorrectAnswers = function() {
	...
	for ( var q = 0; q < testSet.length; q++) {
		    if(testSet[q].className == 'question_wrapper') {
                markCorrectResponse(testSet[q]);
	        }
	}
}

/** Give a list of questoin guesses, mark the correct
 *  one.
 *  
 * @param questionList
 * @return
