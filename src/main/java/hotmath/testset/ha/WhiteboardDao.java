...
public class WhiteboardDao extends SimpleJdbcDaoSupport {
     ...
     * 
     * update whiteboard is tempoary location, using pidEdit as temp key to
     * reference pid.
     * 
    ...
    public void saveStaticWhiteboardData(final String pidEdit, final int teacherId, final String pid,
        ...
        case DELETE:
            ...
            final Integer whiteboardId = getJdbcTemplate().queryForObject(sqlD, new Object[] { pidEdit, rowToDelete },
        ...
        case UNDO:
            ...
            final Integer maxWhiteboardId = getJdbcTemplate().queryForObject(sql, new Object[] { pidEdit },
                    new RowMapper<Integer>() {
                        @Override
                        public Integer mapRow(ResultSet rs, int rowNum) throws SQLException {
                            return rs.getInt(1);
     ...
     * 
     * 
     * TODO: deal with multiple whiteboards per solution, currently only one
     * assumed
     * 
     * @param pid
     * @return
    ...
    public List<WhiteboardModel> getStaticWhiteboards(String pid) throws Exception {
        ...
        List<WhiteboardCommand> list = getJdbcTemplate().query(sql, new Object[] { pid },
                new RowMapper<WhiteboardCommand>() {
                    @Override
                    public WhiteboardCommand mapRow(ResultSet rs, int rowNum) throws SQLException {

                        return new WhiteboardCommand(rs.getString("wb_command"),
                                loadCommandData(rs.getBytes("wb_data")), false);
                    }
                });
    ...
    private String loadCommandData(byte[] commandDataCompressed) throws SQLException {
        try {
            if (commandDataCompressed != null && commandDataCompressed.length > 0
                    && commandDataCompressed[0] != "{".getBytes("UTF-8")[0]) {
    ...
    public void saveWhiteboardAsTemplate(final int uid, String dataUrl) throws Exception {
        ...
        File directory = new File(CatchupMathProperties.getInstance().getSolutionBase() + "/help/whiteboard_template/"
                + uid);
        if (!directory.exists()) {
            directory.mkdirs();
        }
        ...
        final Integer cntMatch = getJdbcTemplate().queryForObject(
                "select count(*) as cnt from CM_WHITEBOARD_TEMPLATE where admin_id = ? and template_name = ?",
                new Object[] { uid, templateName }, new RowMapper<Integer>() {
                    @Override
                    public Integer mapRow(ResultSet rs, int rowNum) throws SQLException {
    ...
    public List<WhiteboardTemplate> getWhiteboardTemplates(int adminId) {
        ...
        List<WhiteboardTemplate> list = getJdbcTemplate().query(sql, new Object[] { adminId },
                new RowMapper<WhiteboardTemplate>() {
                    @Override
    ...
    private void createThumbnail(File imageFile) throws Exception {
        ...
        img.createGraphics().drawImage(ImageIO.read(imageFile).getScaledInstance(1000, 1000, Image.SCALE_SMOOTH), 0, 0,
                null);
    ...
    public void addWhiteboardFigure(final int aid, File file) throws Exception {
        ...
        scaleImageToOptimum(dest);

        final Integer cntMatch = getJdbcTemplate().queryForObject(
                "select count(*) as cnt from CM_WHITEBOARD_TEMPLATE where admin_id = ? and template_name = ?",
                new Object[] { aid, templateName }, new RowMapper<Integer>() {
                    @Override
                    public Integer mapRow(ResultSet rs, int rowNum) throws SQLException {
                        return rs.getInt("cnt");
                    }
    ...
    /**
     * scale image in imageFile to optimize use as whiteboard figure/template
     * 
     * only scale image if size is larger than threshold.
     * 
     * @param imageFile
     */
    private void scaleImageToOptimum(File imageFile) throws Exception {
        ...
        if(width > MAX_WIDTH || height > MAX_HEIGHT) {
            logger.info("Scaling whiteboard figure: " + imageFile.getCanonicalPath());
            BufferedImage scaledImage = Scalr.resize(image, Scalr.Method.SPEED, Scalr.Mode.FIT_TO_WIDTH, MAX_WIDTH, MAX_HEIGHT,Scalr.OP_ANTIALIAS);
            String type = imageFile.getName().split("\\.")[1];
