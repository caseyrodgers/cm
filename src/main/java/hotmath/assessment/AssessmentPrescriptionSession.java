...
public class AssessmentPrescriptionSession {
	 ...
	 */
	public String getInmhReferencedPids(INeedMoreHelpItem item)
			throws HotMathException {
		List<String> list = new ArrayList<String>();

		String pids = "";
		for (SessionData it : this.pids) {
			INeedMoreHelpItem[] i2 = INeedMoreHelpManager.getInstance()
					.getHelpItems(it.getPid(), true);
	...
	public List<INeedMoreHelpResourceType> getInmhTypes() throws Exception {
		...
		INeedMoreHelpResourceType reviewRt = new INeedMoreHelpResourceType(
				INeedMoreHelpResourceTypeDef.RESOURCE_TYPES
		...
		for (INeedMoreHelpItem item : getSessionCategories()) {
			...
			for (INeedMoreHelpItem linked : relatedItems) {
				reviewRt.addResource(new INeedMoreHelpItem(linked.getType(),
						linked.getFile(), linked.getTitle()));
	 ...
	 * 
	 */
	public List<INeedMoreHelpResourceType> getPrescriptionInmhTypes()
			throws HotMathException {
	 ...
	 * @throws Exception
	 */
	public Collection<INeedMoreHelpResourceType> getPrescriptionInmhTypesDistinct()
			throws HotMathException {
		...
		List<INeedMoreHelpResourceType> list = new ArrayList<INeedMoreHelpResourceType>(
				coll);
		Collections.sort(list, new Comparator<INeedMoreHelpResourceType>() {
			@Override
			public int compare(INeedMoreHelpResourceType o1,
					INeedMoreHelpResourceType o2) {
				return o1.getTypeDef().getType().compareTo(
						o2.getTypeDef().getType());
			}
		});
	...
	public List<INeedMoreHelpResourceType> getPrescriptionInmhTypes(
		...
		try {
			...
			String sql = "select distinct m.file,l.link_title, l.link_key,l.link_type "
					...
					+ topicList
					+ ") "
					+ " and l.link_type like '%"
					+ linkTypeIn + "'" + " order by l.link_type, link_title ";
			...
			while (rs.next()) {
				...
				INeedMoreHelpItem inmhItem = INeedMoreHelpItemFactory.create(rs
						.getString("link_type"), rs.getString("link_key"), rs
			...
			throw new HotMathException(e,
					"Error reading prescription inmh items: " + e.getMessage());
		} finally {
