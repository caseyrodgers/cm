...
import hotmath.gwt.hm_mobile.client.activity.Database.CallbackBook;
...
public class BookViewActivity extends AbstractActivity implements BookView.Presenter {
	...
	@Override
	public void loadBookInfo(final String textCode, final CallbackOnComplete callback) {
		...
		Database.getInstance(new Database.Callback() {
			...
			public void dbLoaded(Database database) {
				BookInfoModel bookInfo = database.getBookInfo(textCode);

				int page = bookInfo.getBook().getPage();
				if (page == 0) {
					Integer opage = HmMobilePersistedPropertiesManager.getInstance().getBookPages()
							.get(bookInfo.getBook().getTextCode());
					if (opage != null) {
						page = opage;
					}
				}
				...
				HmMobilePersistedPropertiesManager.setLastBookPlace(bookInfo.getBook(), page);
		 ...
		 * HmMobile.getCmService().execute(action,new AsyncCallback
		 * <BookInfoModel>() { public void onSuccess(BookInfoModel bookInfo) {
		 * 
		 * int page=book.getPage(); if(page == 0) { Integer opage =
		 * HmMobilePersistedPropertiesManager.getInstance().getBookPages().get(
		 * book.getTextCode()); if(opage != null) { page = opage; } }
		 * 
		 * BookView bookView = clientFactory.getBookView();
		 * bookView.showBook(bookInfo.getBook(), bookInfo, page);
		 * 
		 ...
		 * 
		 * 
		 * callback.isComplete(); clientFactory.getEventBus().fireEvent(new
		 * SystemIsBusyEvent(false)); }
		 * 
		 * 
		 * @Override public void onFailure(Throwable arg0) {
		 *           clientFactory.getEventBus().fireEvent(new
		 *           SystemIsBusyEvent(false));
		 * 
		 *           arg0.printStackTrace(); Window.alert(arg0.getMessage()); }
		 *           });
		 */
	}
	...
	@Override
	public void getProblemNumbers(final BookModel book, final int page) {
		...
		Database.getInstance(new Database.Callback() {
			...
			public void dbLoaded(Database database) {
				...
				database.getBookIndex(book.getTextCode(), page, new CallbackBook() {
					@Override
					public void bookLoaded(BookIndex loaded) {
						CmList<ProblemNumber> problemsproblems = loaded.lookupProblemsOnPage(page);
						clientFactory.getBookView().showProblemNumbers(problemsproblems);
						clientFactory.getEventBus().fireEvent(new SystemIsBusyEvent(false));
					}
				});
			}
	...
	public void loadSolution(final ProblemNumber problem) {
		...
		/**
		 * Only allow is is not a free book and user is logged into a non demo
		 * account
		 * 
		 */
		if (!this._lastBookModel.isFree() && HmMobile.__instance.getLoginInfo().isDemoAccount()) {
			PopupMessageBox.showMessage("Hotmath Account Needed",
					"This book requires a paid password. Please visit Hotmath.com for more information.");
			return;
		}
