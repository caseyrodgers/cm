...
public class CustomProblemManager extends GWindow {
	...
	private void buildTreeGrid(List<CustomProblemModel> problems, List<String> paths) {
		...
		_treeTable = new CustomProblemTreeTable(problems, paths,
				new TreeTableCallback() {
	...
	private void buildGui() {
		...
		_emptyPage = new DefaultGxtLoadingPanel(
				"Click on a problem or create a new one");
		...
		_filterButton.addSelectHandler(new SelectHandler() {
			...
			public void onSelect(SelectEvent event) {
				...
				CustomProblemSearchDialog.getInstance(getLessonsInStore(),
						new CustomProblemSearchDialog.Callback() {
							@Override
							public void selectionChanged(
									List<? extends LessonModel> models,
									String comments) {
		...
		TextButton btn = new MyTextButton("Properties", new SelectHandler() {
			@Override
			public void onSelect(SelectEvent event) {
				CustomProblemModel customProblem = _treeTable
						.getSelectedCustomProblem();
				if (customProblem == null) {
					CmMessageBox.showAlert("Please select a problem first.");
					return;
				}
				CustomProblemPropertiesDialog.getInstance(
						new CallbackOnComplete() {
							@Override
							public void isComplete() {
								CmRpcCore.EVENT_BUS
										.fireEvent(new DataBaseHasBeenUpdatedEvent(
												TypeOfUpdate.SOLUTION));
							}
		...
		_showAllTeachers.addChangeHandler(new ChangeHandler() {
			...
			public void onChange(ChangeEvent event) {
				_treeTable.getTree().getStore()
						.setEnableFilters(_showAllTeachers.getValue());
			}
		});
		// flow.add(new MyFieldLabel(_showAllTeachers,"All Teachers",75, 20));

		TextButton selTeach = new TextButton("Select Teacher",
				new SelectHandler() {
					@Override
					public void onSelect(SelectEvent event) {
						new TeacherManager(new Callback() {
		...
		_filter = new StoreFilter<CustomProblemModel>() {
			@Override
			public boolean select(Store<CustomProblemModel> store,
					CustomProblemModel parent, CustomProblemModel item) {
				if (item.getTeacher().getTeacherId() == TeacherManager
	...
	protected void showAddNewFolder(final TeacherIdentity teacher) {
		...
		CmMessageBox.prompt("Create Folder In " + teacher.getTeacherName(), "Folder Name","New Folder", new PromptCallback() {
			@Override
	...
	private void showAddNewProblemDialog() {
		...
		CustomProblemFolderNode folderNode = _treeTable.getSelectedCustomFolderNode();
		
		CustomProblemModel problem = new CustomProblemModel(null, 0,
				TeacherManager.getTeacher(), null, null, folderNode!=null?folderNode.getFolderName():"");
		new CustomProblemPropertyEditor(problem,
				new CustomProblemPropertyEditor.Callback() {
					@Override
	...
	private List<CustomProblemModel> getLessonsOnlyThisAdmin(
			List<CustomProblemModel> all) {
		List<CustomProblemModel> list = new ArrayList<CustomProblemModel>();
		for (CustomProblemModel cp : all) {
			if (cp.getTeacher().getTeacherId() == TeacherManager.getTeacher()
					.getTeacherId()) {
				list.add(cp);
			}
		}
	...
	private void applyFilter(List<? extends LessonModel> models, String comments) {
			...
			for (CustomProblemModel m : list) {
				...
				for (LessonModel lm : m.getLinkedLessons()) {
					...
					for (LessonModel modelToCheck : models) {
						if (modelToCheck.getLessonFile().equals(
								lm.getLessonFile())) {
		...
		if (comments != null) {
			for (int i = listFiltered.size() - 1; i > -1; i--) {
				...
				if (pm.getComments() == null
						|| !(pm.getComments().toLowerCase().contains(comments
								.toLowerCase()))) {
	...
	protected void deleteSelectedNode() {
		...
		if (node instanceof CustomProblemLeafNode) {
			final CustomProblemModel problem = _treeTable
					.getSelectedCustomProblem();
			if (problem == null) {
			...
			CmMessageBox.confirm("Delete Problem",
			...
			if (isCurrentTeacherSubfolder(node)) {
				CmMessageBox.confirm("Delete",	"Delete Folder '" + node.getName() + "' and all problems in folder?", new ConfirmCallback() {
	...
	protected void doDeleteProblem(final CustomProblemModel problem) {
		new RetryAction<RpcData>() {
			...
			@Override
			public void attempt() {
				DeleteCustomProblemAction action = new DeleteCustomProblemAction(
						problem);
				setAction(action);
				CmShared.getCmService().execute(action, this);
			}
	...
	protected void copySelectedProblem() {
		...
		CmMessageBox.confirm("Copy Problem", "Copy problem?",
				new ConfirmCallback() {
					@Override
					public void confirmed(boolean yesNo) {
						if (yesNo) {
	...
	private void doCopySelectProblem(final CustomProblemModel problem) {
		...
		new RetryAction<RpcData>() {
			@Override
			public void attempt() {
				CopyCustomProblemAction action = new CopyCustomProblemAction(
						problem.getPid());
				setAction(action);
				CmShared.getCmService().execute(action, this);
			}
	...
	private void readFromServer() {
		...
		new RetryAction<CustomProblemInfo>() {
			...
			@Override
			public void attempt() {
				GetCustomProblemAction action = new GetCustomProblemAction(
						new TeacherIdentity(
								UserInfoBase.getInstance().getUid(), "TEST", -1));
				setAction(action);
				CmShared.getCmService().execute(action, this);
			}
			...
			public void oncapture(CustomProblemInfo result) {
				CmBusyManager.setBusy(false);
				List<CustomProblemModel> problems = result.getProblems();
	...
	static {
		CmRpcCore.EVENT_BUS.addHandler(DataBaseHasBeenUpdatedEvent.TYPE,
				new DataBaseHasBeenUpdatedHandler() {
					@Override
