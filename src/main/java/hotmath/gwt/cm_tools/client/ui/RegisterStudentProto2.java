...
public class RegisterStudentProto2 extends LayoutContainer implements ProcessTracker {
	...
	private ListStore <ProgramTypeModel> progTypeStore;
	private ComboBox<ProgramTypeModel> progTypeCombo;
	...
	private ComboBox<StudyProgramExt> quizCombo;	
	...
	public RegisterStudentProto2(StudentModelI sm, CmAdminModel cm) {
        this(sm, cm, false);
	}

	public RegisterStudentProto2(StudentModelI sm, CmAdminModel cm, boolean excludeAutoEnroll) {
	...
	protected FormPanel createForm() {
		...
		initProgramTypeList();
		progTypeCombo = programTypeCombo(progTypeStore, _fsProgram);
		_fsProgram.add(progTypeCombo);
    ...
    private void setupStdProgramUI() {
        ...
        progCombo = programCombo(stdProgStore, _fsStdProg);
		_fsStdProg.add(progCombo);
    ...
    private void setupCustomQuizUI() {
        _fsCustomQuiz = new FieldSet();
        _fsCustomQuiz.setHeading("");
        _fsCustomQuiz.addStyleName("register-student-inner-fieldset");
        FormLayout fl = new FormLayout();
		fl.setLabelWidth(_formPanel.getLabelWidth());
		fl.setDefaultWidth(LAYOUT_WIDTH);
		_fsCustomQuiz.setLayout(fl);
	...
	private ComboBox<ProgramTypeModel> programTypeCombo(ListStore<ProgramTypeModel> store, final FieldSet fs) {
		ComboBox<ProgramTypeModel> combo = new ComboBox<ProgramTypeModel>();
		...
		combo.setId("prog-type-combo");
		combo.setTypeAhead(true);
		combo.setSelectOnFocus(true);
		combo.setEmptyText("-- select program type --");
		combo.setWidth(280);

	    combo.addSelectionChangedListener(new SelectionChangedListener<ProgramTypeModel>() {
	        @SuppressWarnings("unchecked")
			public void selectionChanged(SelectionChangedEvent<ProgramTypeModel> se) {

	        	if (loading) return;

	            ProgramTypeModel sp = se.getSelectedItem();
	            CmProgramType type = sp.getType();
	            ...
	            case CUSTOM:
	            	...
	            	break;
	            case CUSTOMQUIZ:
	        	    cardPanel.setActiveItem(_fsCustomQuiz);
	            	break;
	            case PROF:
	        	    cardPanel.setActiveItem(_fsStdProg);
	            	break;
			...
			public void selectionChanged(SelectionChangedEvent<SubjectModel> se) {
	        	...
	        	if (subjectId == null || ! subjectId.equals(sm.getAbbrev())) {
	        	    try {
    		        	...
    		        	ComboBox<StudyProgramExt> cb = (ComboBox<StudyProgramExt>) _fsStdProg.getItemByItemId("prog-combo");
	...
	private void initProgramTypeList() {
		progTypeStore = new ListStore <ProgramTypeModel> ();
        List<ProgramTypeModel> list = new ArrayList <ProgramTypeModel> ();
        list.add(new ProgramTypeModel("Standard Program", CmProgramType.PROF));
        list.add(new ProgramTypeModel("Custom Program", CmProgramType.CUSTOM));
        list.add(new ProgramTypeModel("Custom Quiz", CmProgramType.CUSTOMQUIZ));
        progTypeStore.add(list);
	}
