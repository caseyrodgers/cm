...
/**
 * Create a button with an optional attached menu to display CM resources where
 * the proper placement is calculated dynamically and positioned correctly in
 * the available space
 * 
 * 
 * @author casey
 *
 */
public class ResourceMenuButton extends TextButton {
    ...
    public ResourceMenuButton(final PrescriptionSessionDataResource resource, RegisterCallback callback) {
        ...
        if (resource.getSubMenuItems().size() > 0) {
            ...
            if (isResourceAvailable(resource)) {
                ...
                this.disable();
                // this.setToolTip("Sorry, " + resource.getLabel() +
                // " are not available.");
            }
        } else {
            ...
            addSelectHandler(new SelectHandler() {
                ...
                public void onSelect(SelectEvent event) {
                    registerCallback.loadResourceIntoHistory(resource.getType().label(), "0");
        ...
        /**
         * Register resource items for this type. This allows the history
         * mechanism to locate the proper resource to show.
         * 
         */
        registerCallback.registerItems(resource.getType().label(),  resouresToRegister);
     ...
     * 
     * This is to deal with GXT adding extra length to buttons with menu.
     * 
     */
    ...
    /**
     * Check to see if this resource has been completed, if so then indicate
     * appropriately.
     */
    ...
    /**
     * Provide any last minute modification to the list items Such as assigning
     * unique titles, as with the the cmextra
     * 
        ...
        for (final InmhItemData id : resource.getItems()) {
            ...
            item.addSelectionHandler(new SelectionHandler<Item>() {
                ...
                public void onSelection(SelectionEvent<Item> event) {
                     ...
                     * 
                     * @TODO: This needs to be centralized (throw a CLOSE
                     *        event?)
                     */
                    ...
                    registerCallback.loadResourceIntoHistory(resource.getType().label(), ordinalPosition.toString());
                }
            });
        }
    ...
    /**
     * Provide a method to have submenus. If a submenu has a null title, then
     * show the first item as the a normal MenuItem without a submenu.
     * 
     * @param resource
     * @return
     */
    ...
    private void resourceWasClicked(InmhItemData id) {
        ...
        for (InmhItemData idI : resouresToRegister) {
            if (idI.getFile().equals(id.getFile())) {
                if (isResourceAvailable(resource)) {
                    registerCallback.loadResourceIntoHistory(resource.getType().label(), pos.toString());
                    return;
                }
            }
            pos++;
